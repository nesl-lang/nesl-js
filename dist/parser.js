...
const validateHeredocDelimiter_1 = require("./validators/validateHeredocDelimiter");
const parseHeader_1 = require("./parsers/parseHeader");
const parseEndMarker_1 = require("./parsers/parseEndMarker");
const parseAssignment_1 = require("./parsers/parseAssignment");
 ...
 */
function parseNesl(content) {
    const lines = content.split(/\r?\n/);
    let state = 'SEEKING_HEADER';
    const blocks = [];
    const errors = [];
    let currentBlock = null;
    let heredocInfo = null;
    // Helper to add error with context
    const addError = (code, lineNum, message, column = 0, length) => {
        ...
        errors.push({
            ...
            content: line,
            context: (0, getContextWindow_1.getContextWindow)(content, lineNum),
            message
        });
    };
    // Process each line
    for (let i = 0; i < lines.length; i++) {
        const lineNum = i + 1;
        const line = lines[i];
        const lineType = (0, classifyLine_1.classifyLine)(line);
        switch (state) {
            case 'SEEKING_HEADER': {
                if (lineType === 'header') {
                    const headerResult = (0, parseHeader_1.parseHeader)(line);
                    if (headerResult.isValid && headerResult.blockId) {
                        const validation = (0, validateBlockId_1.validateBlockId)(headerResult.blockId);
                        if (validation.valid) {
                            currentBlock = {
            ...
            case 'IN_BLOCK': {
                ...
                if (lineType === 'end_marker') {
                    const endResult = (0, parseEndMarker_1.parseEndMarker)(line);
                    if (endResult.isEnd && endResult.blockId) {
                        if (endResult.blockId === currentBlock.id) {
                            ...
                            addError('MISMATCHED_END', lineNum, `End marker '${endResult.blockId}' doesn't match block ID '${currentBlock.id}'`);
                    ...
                    addError('UNCLOSED_BLOCK', lineNum - 1, `Block '${currentBlock.id}' not closed before new block`, 0, 0);
                    ...
                    const assignment = (0, parseAssignment_1.parseAssignment)(line);
                    if (assignment.success) {
                        if (assignment.type === 'key-value') {
                            ...
                            const keyValidation = (0, validateKey_1.validateKey)(assignment.key);
                            if (!keyValidation.valid || hasLeadingWhitespace) {
                                const invalidChar = (0, findInvalidCharPosition_1.findInvalidCharPosition)(assignment.key);
                                if (hasLeadingWhitespace) {
                                    ...
                                    addError('INVALID_KEY', lineNum, keyValidation.error || 'Invalid key', 0, assignment.key.length);
                                }
                            }
                            else if (assignment.key in currentBlock.properties) {
                                addError('DUPLICATE_KEY', lineNum, `Duplicate key '${assignment.key}' in block '${currentBlock.id}'`, 0, assignment.key.length);
                            ...
                            if (keyValidation.valid) {
                                const delimiterValidation = (0, validateHeredocDelimiter_1.validateHeredocDelimiter)(assignment.delimiter, currentBlock.id);
                                if (delimiterValidation.valid) {
                                    ...
                                    addError('INVALID_HEREDOC_DELIMITER', lineNum, delimiterValidation.error || 'Invalid heredoc delimiter', delimiterIndex >= 0 ? delimiterIndex : 0, assignment.delimiter.length);
                                }
                            }
                        }
                    }
                        ...
                        if (error.code === 'INVALID_OPERATOR') {
                            ...
                            addError('INVALID_ASSIGNMENT_OPERATOR', lineNum, `Invalid assignment operator '${operator}' - only '=' is allowed`, error.position, error.length);
                        }
                            ...
                            if (afterEqual.startsWith('"')) {
                                ...
                                if (i < afterEqual.length && afterEqual[i] === '"') {
                                    try {
                                        ...
                                        const keyValidation = (0, validateKey_1.validateKey)(keyPart);
                            ...
                            addError(error.code, lineNum, getErrorMessage(error.code, line, currentBlock?.id), error.position, error.length);
                        }
                        else {
                            addError(error.code, lineNum, getErrorMessage(error.code, line, currentBlock?.id), error.position, error.length);
                        }
                    ...
                    addError('MALFORMED_ASSIGNMENT', lineNum, `Invalid line format in block '${currentBlock.id}': not a valid key-value assignment or empty line`);
                }
                break;
            }
            case 'IN_HEREDOC': {
                if (!heredocInfo || !currentBlock)
                    break;
    ...
    if (state === 'IN_HEREDOC' && heredocInfo && currentBlock) {
        addError('UNCLOSED_HEREDOC', lines.length + 1, `Heredoc '${heredocInfo.delimiter}' not closed before EOF`, 0, 0);
        // Save partial block
        currentBlock.endLine = null;
        blocks.push(currentBlock);
    }
    else if (state === 'IN_BLOCK' && currentBlock) {
        addError('UNCLOSED_BLOCK', lines.length + 1, `Block '${currentBlock.id}' not closed before EOF`, 0, 0);
        currentBlock.endLine = null;
        blocks.push(currentBlock);
    }
 ...
 */
function getErrorMessage(code, line, blockId) {
    switch (code) {
        case 'EMPTY_KEY':
